...
### HEAD - всему голова.
*HEAD* - один из служебных файлов папки .git. Он указывает на коммит, который был сделан последним (самый новый).
Если вывести в консоль последние HEAD при помощи cat, то там будет ссылка на служебный файл (refs/heads/master).
Если же заглянуть в этот файл, то можно увидеть хеш последнего коммита.
Когда делается новый коммит - git обновляет refs/heads/master - записывает в него хеш последнего коммита. Соответственно, HEAD тоже обновляется.
...
и .git поймет, что мы обращаемся к последнему коммиту.