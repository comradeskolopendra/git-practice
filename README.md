...
Две ветки находятся в состоянии fast-forward, если:
- При слиянии этих двух веток никак не возможен конфликт.
- Одна ветка является продолжением другой.
- Истории коммитов двух веток никак не разошлись.

После слияния веток, с помощью команды `git merge <branch-name>`, стоит обратить внимание на:
- если рабочая ветка продолжает историю целевой - в консоли будет указано `Fast-forward`.
- В истории коммитов - последний коммит (HEAD) указывает одновременно и на рабочую ветку и на целевую ветку.

Если слияние происходит в режиме `fast-forward`, то git просто добавляет коммиты из рабочей ветки в целевую.

Слияние веток в режиме `fast-forward` можно отключить с флагом `--no-ff` в команде `git merge <branch-name>`:
```bash
git merge --no-ff <branch-name>
```

Так же режим `fast-forward` можно отключить навсегда с помощью настройки в конфиге:
```bash
git config [--global] merge.ff false
```

Если отключить слияния в режиме `fast-forward` - то git создаст в целевой ветке мерж-коммит:
```bash
# no-edit избавляет от необходимости ввести сообщение для мерж-коммита.
# no-ff отключает режим слияния fast-forward
# выведет другой режим слияния: Merge made by the "ort" strategy.
git merge --no-edit --no-ff <branch-name>

# --graph нарисует ветки в коносоли с помощью "звездочек" и "палочек"
# получившийся коммит слияния окажется в целевой ветке с сообщением <commit-hash> (HEAD -> (main | master)) Merge branch <work-branch-name>
git log --oneline --graph
```

## Non-fast-forward
При слиянии веток в режиме `non-fast-forward` - Git обязательно создает коммит слияния в целевой ветке.

Когда Git проверяет ветки на состояние `fast-forward`, он не заглядывает внутрь файлов и не пытается угадать, будет ли конфликт на самом деле. Для Git важно только, что конфликт теоретически возможен, или невозможен.

Если конфликтов при слиянии нет - команда `git merge` отработает почти автоматически, только предложит ввести сообщение для мерж-коммита, но можно и не вводить и оставить сообщение, которое предложил Git - для этого нужно в команде `git merge` добавить флаг `--no-edit`.

Если конфликты есть - гит попытается решить их сам. Если не получится - он предложит решить их вручную.

При слиянии в режиме `non-fast-forward` могут быть **(НО НЕ ОБЯЗАТЕЛЬНЫ)** конфликты.

## git push и fast-forward, git push и non-fast-forward.
Команда `git push` толкает коммиты из локальный ветки в её даленную комию.

Хоть `git push` и не выполняет слияния веток, для этой команды тоже важно состояние fast-forward: между локальной веткой (откуда берутся коммиты) и удаленной веткой (куда попадут коммиты).

Ветки с суффиксом "@origin" - в контексте данного конспекта темы означают, что это удаленные ветки, а без суффикса - локальные.

Если main и main@origin находятся между собой в состоянии fast-forward, то `git push` перематывает удаленную ветку до состояния локальной (то есть, добавляет коммиты в удаленную ветку из локальной). В итоге main и main@origin будут содержать одинаковый набор коммитов.

---

Если попытаться выполнить команду `git push` на ветке, которая находится в состоянии `non-fast-forward`, то он выведет ошибку в консоль. В ошибке стоит обратить внимание на надписи:
- `[rejected]` - выполнение команды push отклонено.
- `main -> main (non-fast-forward)` - ветка находится в состоянии `non-fast-forward`.

В git можно решить проблему разошедшихся веток с помощью операции rebase (от англ. перебазирование). Эта операция позволяет изменить коммит (точку) от которого делалась ветка.
После перебазирования ветки будут в состоянии fast-forward, что позволит без ошибок загрузить коммиты из локальной ветки в удаленную ветку. Однако при перебазировании могут быть плохие последствия:
- могут возникать конфликты между изменениями, как при слиянии веток.
- если действовать неаккуратно, то можно "сломать" репозиторий.

Есть другой способ справиться с ошибкой `reject: non-fast-forward` - форсированный пуш. Чтобы выполнить такой пуш, к команде `git push` добавляют флаг `--force`.
При таком пуше из удаленной ветки выкидываются параллельные коммиты и записываются коммиты из локальной ветки

**NOTE:** `git push --force` **уместная команда только в ОЧЕНЬ редких случаях!**

## Почему бы не "пушить" все в main.

Когда над проектом работает команда, тактика "пушить все в main" становится плохой:
- Потому что новые сотрудники ещё могли не влиться полностью в процесс и случайно запушить что-то не то в прямо в main.
- `git push` ожидает, что обновление будет происходить в режиме fast-forward. Добиться такого статуса при активной работе нескольких человек над одной веткой достаточно сложно.

`git push` произойдет без каких-либо проблем, если локальная и удаленная ветки не разошлись, и одна сможет догнать другую без конфликтов.
Если несколько человек будут пушить коммиты прямо в main, локальная ветка main и удаленная ветка main будут постоянно расходится у других учатсников команды.

## Модели веток. Простая feature branch модель.
Подходы к работе с ветками - это правила, которые описывают, когда и для чего создаются ветки, какие в них коммиты и в какой они сливаются.

Подходы, так же могут называть workflow, или сокращенно: flow.
Три самых популярных подхода:
- Feature-branch workflow - простой и самый пополуярный подход. В нем, для каждого изменения создается отдельная ветка, которая позже мержится с main.
- Git flow - похож на feature-branch workflow, но отличается тем что создается больше веток, а коммиты делят на разные типы: исправление, добавление нового функционала и т.д., и коммиты расфасованы по веткам.
- Trunk-based - Участники репозитория вливают изменения в основную ветку максимально часто, например - каждый день.

Основная идея feature-branch worklflow в том, что каждая новая функциональность, исправление багов ведется в отдельно ветке.
Когда процесс разработки в созданной ветке подошел к логическому завершению - ветка сливается с main и удаляется.
Такой подход позволяет разработчикам работать над новой функциональностью независимо от других разработчиков, и не затрагивает основную ветку кода в процессе.

Если аккуратно использовать модель feature-branch workflow, то:
- Не будет проблемы расхождающихся веток, ведь новые изменения попадают в main не через пуш, а через мерж. Команду `git merge` разошедшиеся ветки не смущают, ведь для них придумана.
- В ветке main всегда полностью рабочий функционал, все недоделанные фичи остаются в feature/ ветках до момента, пока не будут готовы к мержу.

Feature-branch workflow это шаблон модели, а не строгий алгоритм. В некоторых командах разработки из данной ветки убирают некие правила, в некоторых командах добавляют, но основа, которая строится на множестве feature/ веток, которые вливают в одну основную - main, неизменна.

## Pull request и code review.
Перед слиянием - любая ветка проходит разные проверки, в том числе - другими членами команды.

Новые функциональности или исправления попадают в main путем слияния **"запрос на слияние", который так же называют пулл-реквест ИЛИ мерж-реквест**.

На этапе пулл-реквеста могут быть разные проверки:
- запуск автоматических тестов, которые покажут, сломают ли новые изменения существующую в проекте логику.
- просмотр кода "глазами", или как ещё это называется - review.

Ревью важно, потому что:
- проверяется качетсво и необходимости вносимых изменений.
- те, кто выполняют ревью - узнают больше о вносимой в проект функциональности.

Критерии для оценки изменений:
- соответствует ли общему стилю проекта.
- нет очевидных уязвимостей.
- нет ничего лишнего в написанном коде.

Если в PR все хорошо - ревьювер нажимает кнопку Approve (от англ. "одобрить") или оставляется комменатрий LGTM (от англ. "Looks Good To Me" - "На мой взгляд, все хорошо"). После одобрения - автор или любой другой участник репозитория - может нажать кнопку Merge, и тогда GitHub вольет изменения в целевую ветку,  а так же может предложит удалить рабочую ветку, так как изменения были влиты в целевую.

## Работа с PR: Практика.
Чтобы связать локальнкую и удаленную ветки можно использовать команду - `git push --set-upstream origin HEAD`. Данная команда аналогична - `git push -u origin <branch-name>`. `--set-upstream` - полная запись флага `-u`, а `HEAD` - синоним текущей ветки.

Если связать ветки такимо образом между собой - в будущем можно будет не уточнять при `git push` и `git pull` куда пушить или откуда пуллить изменения.

После мержа PR - изменения вливаются только в удаленную целевую ветку. После мержа PR стоит забирать эти изменения в свою локальную ветку, командами:
```bash
